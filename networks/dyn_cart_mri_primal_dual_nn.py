...
class DynCartPrimalDualNN(nn.Module):
    def __init__(
        self,
        EncObj,
        T=128,
        ...
        super(DynCartPrimalDualNN, self).__init__()
    ...
    def forward(self, y, x, mask, csmap=None, lambda_map=None):
        ...
        for kT in range(self.T):
            # update p
            p = (p + sigma * (self.EncObj.apply_A(xbar, csmap, mask) - y)) / (
                1.0 + sigma
            )
            ...
            x1 = (
                x0
                - tau * self.EncObj.apply_AH(p, csmap, mask)
                - tau * self.GradOps.apply_GH(q)
            )
