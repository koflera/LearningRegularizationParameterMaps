...
def generate_2d_csmaps(im_size, ncoils, coil_width = 64):
	"""
	generate a coil sensititivy map of shape Nx,Ny,Nz, i.e. a tensor
	(1,Nx,Ny,Nz)
	"""
	csmap = np.stack(np.abs(mrisensesim(im_size, ncoils=ncoils, coil_width=coil_width)))
	csmap = torch.tensor(csmap).unsqueeze(0)
	
	#normalize csms according to
	#https://onlinelibrary.wiley.com/doi/epdf/10.1002/mrm.24751, after eq 2.
	norm_factor = torch.pow( torch.sum(csmap.conj() * csmap, dim=1, keepdim=True), -0.5)
	
	return (norm_factor * csmap).to(torch.complex64)
